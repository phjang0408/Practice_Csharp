...
namespace DrawPaper
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // 2. ==, != 연산자 : 모든 데이터 형식 가능 
            // -> 클래스도 가능하지만, 참조값 비교이므로, 같은 멤버값을 가진 객체라도 false 반환
            //   
            // 3. null 조건부 연산 "?." 과 "?[]"
            ...
            // 4. null 병합 연산자 "??"
            // A ?? B
            // A가 null이 아님 => A 반환, A가 null => B 반환 
            int? num = null;
            Console.WriteLine(num ?? 123);      // null이라 123반환

            num = 99;
            Console.WriteLine($"{num ?? 123}"); // null이 아니라, 자기자신 반환

            string s = null;
            Console.WriteLine(s ?? "It is NULL!");
